...
describe('Testing endpoints for transaksi-detail', () => {
    describe('should get all transaksi-detail', () => {
        it('responds with json', async () => {
            const response = await (0, supertest_1.default)(__1.default).get('/transaksi-detail').set('Accept', 'application/json');
            expect(response.status).toBe(200);
        });
    });
    // ## describe untuk GET USERS BY ID
    describe('GET /transaksi-detail/:id', () => {
        it('should get user by ID', async () => {
            const Id = '48eb6c5a-92d3-41ee-8d5f-984038b80db2'; // ID pengguna yang ingin Anda uji
            const response = await (0, supertest_1.default)(__1.default).get(`/transaksi-detail/${Id}`);
            expect(response.status).toBe(200);
        ...
        it('should return 404 if user is not found', async () => {
            ...
            const response = await (0, supertest_1.default)(__1.default).get(`/transaksi-detail/${nonExistingUserId}`);
            expect(response.status).toBe(200);
