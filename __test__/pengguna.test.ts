...
describe('Testing endpoints for user', () => {
  describe('should get all users', () => {
    it('responds with json', async () => {
      const response = await supertest(app).get('/pengguna').set('Accept', 'application/json');
  ...
  describe('GET /users/:id', () => {
    it('should get user by ID', async () => {
      ...
      const response = await supertest(app).get(`/pengguna/${Id}`);
      expect(response.status).toBe(200);
    ...
    it('should return 404 if user is not found', async () => {
      ...
      const response = await supertest(app).get(`/pengguna/${nonExistingUserId}`);
  ...
  describe('POST /users', () => {
    it('should create a new user', async () => {
      ...
      const response = await supertest(app).post('/pengguna').send(newUser);
  ...
  describe('PUT /pengguna/:id', () => {
    it('should update an existing user', async () => {
      ...
      const response = await supertest(app).patch(`/pengguna/${Id}`).send(updatedUser);
    ...
    it('should return 404 if user is not found', async () => {
      ...
      const response = await supertest(app).put(`/pengguna/${nonExistingUserId}`).send(updatedUser);
