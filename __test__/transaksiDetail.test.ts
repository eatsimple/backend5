...
describe('Testing endpoints for transaksi-detail', () => {
  describe('should get all transaksi-detail', () => {
    it('responds with json', async () => {
      const response = await supertest(app).get('/transaksi-detail').set('Accept', 'application/json');
  ...
  describe('GET /transaksi-detail/:id', () => {
    it('should get user by ID', async () => {
      ...
      const response = await supertest(app).get(`/transaksi-detail/${Id}`);
      expect(response.status).toBe(200);
    ...
    it('should return 404 if user is not found', async () => {
      ...
      const response = await supertest(app).get(`/transaksi-detail/${nonExistingUserId}`);
