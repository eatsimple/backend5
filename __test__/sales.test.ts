...
describe('Testing endpoints for sales', () => {
  describe('should get all sales', () => {
    it('responds with json', async () => {
      const response = await supertest(app).get('/sales').set('Accept', 'application/json');
  ...
  describe('GET /sales/:id', () => {
    it('should get user by ID', async () => {
      ...
      const response = await supertest(app).get(`/sales/${Id}`);
      expect(response.status).toBe(200);
    ...
    it('should return 404 if user is not found', async () => {
      ...
      const response = await supertest(app).get(`/sales/${nonExistingUserId}`);
